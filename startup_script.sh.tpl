...
else if [ $SPLUNK_ROLE = "SHC-Member" ]; then
  # Configure some SHC parameters
  cat >>$SPLUNK_HOME/etc/system/local/server.conf <<end
[shclustering]
register_replication_address = $LOCAL_IP
  ...
  sudo -u $SPLUNK_USER $SPLUNK_BIN init shcluster-config -mgmt_uri https://$LOCAL_IP:8089 -replication_port 8090 -replication_factor 2 -conf_deploy_fetch_url https://${SPLUNK_SHC_DEPLOYER_PRIVATE_IP}:8089 -shcluster_label Splunk-SHC -secret ${SPLUNK_CLUSTER_SECRET} -auth admin:${SPLUNK_ADMIN_PASSWORD}
...
else if [ $SPLUNK_ROLE = "IDX-Peer" ]; then
